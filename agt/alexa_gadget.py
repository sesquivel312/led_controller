...
class AlexaGadget:
    ...
    def __init__(self):
        ...
        # self._load_gadget_config(gadget_config_path) # now config is in self.config - from yaml - including what was in ini & agt.json
        ...
        self._peer_device_bt_addr = self.config.get('echo_address', None)
        # Get the radio address
        self._transport_mode = self.config.get('transport_mode', None)
        self._transport_mode = self.config['agt']['transport_mode']
